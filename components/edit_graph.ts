...
import { addLink, addNode, containsLink, deleteLink, deleteNode, iGraphAfterRender } from '../utils/updateGraph';
        ...
        if (_changedProperties.has('event')) {
            ...
            if (e.detail.type == 'NODE') {
                if (this.action == 'delete') {
                    (this.graph as unknown as iGraphAfterRender) = deleteNode(this.graph, e.detail.data.name);
                    this.action = '';
                    this.updateGraph();
                    document.body.style.cursor = 'auto';
                    return;
                ...
                if (this.action == 'add link 2') {
                    if (
                        ...
                        this.graph = addLink(this.graph, this.selectedNode, e.detail.data.name, this.newLinkWeight);
                    }
                    this.graph.newLink = '';
                    this.updateGraph();
                    this.action = '';
                ...
                <tr>
                    <td>
                        <div class="buttonAndInput">
                            <sl-input @input="${(e) => (this.newNode = e.target.value)}" label="Name"></sl-input>
                            <sl-tooltip
